...
import { AlertTriangle, CheckCircle, XCircle, Share2, ArrowRight, Search, ChevronDown } from "lucide-react";
...
export default function BanChecker() {
  ...
  const fetchPopularSubreddits = async () => {
    try {
      ...
      const response = await fetch("https://www.reddit.com/subreddits/popular.json?limit=100");
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (selectedIndex >= 0 && filteredSubreddits[selectedIndex]) {
          setSubreddit(filteredSubreddits[selectedIndex].display_name);
          setSearchQuery(filteredSubreddits[selectedIndex].display_name);
          setIsDropdownOpen(false);
        }
        break;
      case "Escape":
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <a
            href={process.env.NEXT_PUBLIC_MAIN_APP_URL || "https://unbannnable.com"}
            target="_blank"
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Free instant AI analysis. Check if your post violates Reddit rules
                  ...
                  {loadingSearch ? (
                    ...
                    filteredSubreddits.map((sr, index) => (
                      <div
                        key={sr.id}
                        onClick={() => {
                          setSubreddit(sr.display_name);
                          setSearchQuery(sr.display_name);
                          setIsDropdownOpen(false);
                        }}
                      ...
                      >
                        ...
                        {sr.public_description && (
                          <div className="text-sm text-gray-500 truncate">
                            {sr.public_description.substring(0, 80)}
                            {sr.public_description.length > 80 ? "..." : ""}
                          </div>
                        )}
                        <div className="text-xs text-gray-400 mt-1">
                          {sr.subscribers.toLocaleString()} members
                        </div>
                      </div>
                    ))
                    ...
                    <div className="px-4 py-8 text-center text-gray-500">
                      ...
                      {allSubreddits.length > 0 && (
                        <p className="text-xs mt-2">Or select from popular subreddits below</p>
                      )}
                    </div>
                  )}
                </div>
            ...
            >
              ...
              >
                ...
                <p className="text-white/90 mb-6">
                  Unbannnable uses AI to rewrite your post, check all subreddit
                  rules, and ensure you never get banned again.
                </p>
